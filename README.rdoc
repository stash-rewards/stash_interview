...
for our members - promotion to them the hotels that they're most likely to be
interested in based on their past behavior.
...
3. Clone this repo from github: git clone https://github.com/daws/stash_interview.git
...
this project will be to create a basic algorithm:
  ...
  For a given member and date, pick a hotel to recommend to the member based on
  their past behavior.

The UI doesn't matter, but we want a way to test it out. For example, a form that
takes a member and a date and returns the name of the recommended hotel would
be fine. Some basic unit tests would be a plus.

We *DO NOT* want to consider behavior of other members in the network (ie.
recommend hotels based on behavior of similar users. This will be a great feature
in the future, but we want the scope/complexity limited for this first iteration.
...
included) and email it to david@stashrewards.com. If you don't finish a certain
part, have warnings/gotchas about your solution, or have any thoughts on what
you'd like to improve next if this were a real life project, feel free to
include some notes.
